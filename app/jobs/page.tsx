...
function ScorePill({ score }: { score: number }) {
  ...
  return (
    <span className={`text-xs font-bold border rounded-full px-2.5 py-0.5 shrink-0 ${color}`}>
      {score}/10
    </span>
  )
}
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 rounded-xl border border-input bg-background text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent transition-all"
          />
          {localSearch && (
            <button
              onClick={() => {
        ...
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          ...
          {activeFilters.length > 0 && (
            <button
              onClick={() => updateParams({ sector: null, remote: null, q: null })}
        ...
        {/* Sector chips dropdown */}
        {sectorsOpen && (
          <div className="bg-background border rounded-xl shadow-md p-4 mb-3 flex flex-wrap gap-2">
            {SECTORS.map(s => (
              <button
                key={s.id}
                onClick={() => {
                  updateParams({ sector: s.id === 'all' ? null : s.id })
          ...
          <div className="space-y-3">
            {jobs.map(job => (
              <Link
                ...
                href={
                  isLoggedIn
                    ? `/jobs/${job.id}?back=${encodeURIComponent(pathname + (searchParams.toString() ? '?' + searchParams.toString() : ''))}`
                    : '/signup'
                }
                className="block rounded-xl border bg-card hover:border-foreground/20 hover:shadow-md transition-all duration-150"
              >
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-2 mt-3 flex-wrap">
                    ...
                    <span className="text-xs border rounded-full px-2 py-0.5 text-muted-foreground bg-muted/40">
                      {SOURCE_LABELS[job.source] ?? job.source}
                    </span>
                    {job.is_remote && (
                      <span className="text-xs border rounded-full px-2 py-0.5 text-primary bg-primary/5 border-primary/20">
                        Remote
                      </span>
                    )}
                    {job.posted_at && (
